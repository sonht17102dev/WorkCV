...
<body>
			...
			<div
				...
				<div class="col-md-12 text-center mb-5">
					<p class="breadcrumbs mb-0">
						...
						</a></span> ${action.equals("add") ? "Đăng bài tuyển dụng" : "Cập nhật bài đăng"}
						<span></span>
					</p>
					<h1 class="mb-3 bread">${action.equals("add") ? "Đăng bài tuyển dụng" : "Cập nhật bài đăng"}</h1>
				</div>
			</div>
		</div>
	</div>
	...
	<section class="section-hero overlay inner-page bg-image"
		...
		<div class="container">
			<div class="row">
				<div class="col-md-7">
					<h1 class="text-white font-weight-bold">${action.equals("add") ? "Đăng bài tuyển dụng" : "Cập nhật bài đăng"}</h1>
					<div class="custom-breadcrumbs">
						<a href="${contextPath}/">Trang chủ</a> <span class="mx-2 slash">/</span>
						<span class="text-white"> <strong>${action.equals("add") ? "Đăng bài tuyển dụng" : "Cập nhật bài đăng"}</strong>
						</span>
					</div>
				</div>
			</div>
	...
	<section class="site-section">
		<div class="container">
			<form:form action="${contextPath}/recruitment/post" method="post"
				modelAttribute="recruitmentDTO">
				<form:input type="hidden" path="userId" value="${userLogin.id}" />
				<form:input type="hidden" path="id" />
				<input type="hidden" name="action" value='${action.equals("add") ? "add" : "update"}'/>
				
				<div class="row align-items-center mb-5">
					<div class="col-lg-8 mb-4 mb-lg-0">
						<div class="d-flex align-items-center">
							<div>
								<h2>${action != null && action.equals("add") ? "Đăng bài tuyển dụng" : "Cập nhật bài đăng"}</h2>
							</div>
						</div>
					</div>
						...
						<div class="p-4 p-md-5 border rounded" method="post">
							...
							<div class="form-group">
								...
								<form:textarea path="description" class="form-control"
									id="editorN" placeholder="Mô tả công việc" />
							...
							<div class="form-group">
								...
								<form:input type="date" class="form-control" path="deadline"
									placeholder="Địa chỉ công ty" />
	...
	<script>
	    ClassicEditor.create(document.querySelector('#editorN')).then(eidt => {
	        //eidt.setData("${recruitment.description}")
	    })
	        .catch(error => {
	            console.error(error);
	        });
